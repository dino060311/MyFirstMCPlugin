...
- **바닐라 타겟 셀렉터 통합:** `Bukkit.selectEntities()` API를 활용하여 `@a`, `@p`, `@r` 및 특정 닉네임까지 마인크래프트 표준 대상 지정 방식을 완벽하게 지원합니다. 또한 인자 미입력 시 명령어를 실행한 본인에게 즉시 적용되도록 설계하여 운영 편의성을 극대화했습니다.
- **유연한 엔티티 필터링:** 셀렉터로 선택된 대상 중 `instanceof Player` 체크를 통해 실제 플레이어에게만 컷신 로직이 실행되도록 안전 장치를 마련했습니다.
- **상태 제어 및 행동 통제:** `HashSet<UUID>`을 활용해 상태 제어 대상자를 관리하고, `PlayerMoveEvent`에서 시선(Yaw/Pitch) 및 이동(X/Y/Z) 변화를 감지하여 완벽하게 행동을 통제(`setCancelled`)합니다.
...
### 13. 애니메이션 기반 직업 추첨 및 자동 배정 시스템 (Roulette & Auto-Assignment)

- 단순히 명령어로 직업을 정하는 방식을 넘어, **GUI 애니메이션**을 활용하여 시청각적 재미를 극대화한 추첨 시스템을 구현했습니다.
- **서비스 계층 분리**: 비즈니스 로직(JobService), 시각적 연출(RouletteService), 데이터 관리(RoleManager)를 각각 독립된 클래스로 설계하여 유지보수성과 코드 재사용성을 극대화했습니다.
...
- **객체 생명주기 관리 및 의존성 관리**: 각 클래스가 필요로 하는 데이터(Manager, Service 등)를 `InitManager`를 통해 안전하게 주입하는 구조를 설계하여 데이터 충돌 및 서버 불안정성을 방지했습니다.
- **역할 기반의 계층형 구조 도입**:
...
### 17. Skript 기반 PvP 미니게임 시스템 구현 (가위바위보 데스매치)
...
- **예외 처리 및 UX 보완:** 미선택 자동 패배, 무승부 재경기, 자동 체력 회복 차단, 사망 시 게임 완전 종료 등 실제 멀티플레이 환경에서 발생 가능한 예외 케이스를 고려해 안정성을 확보했습니다.
...
### 18. 보스 레이드 시스템 및 API 최신화

- **객체 지향 아키텍처 설계:** 복잡한 보스 몬스터 시스템을 구현하기 위해 보스 소환의 진입점을 담당하는 `BossSpawnCommand`, 보스의 상태와 보스바를 제어하는 `BossManager`, 페이즈별 특수 스킬을 담당하는 `BossSkillService`, 타격 이벤트를 처리하는 `BossListener`로 클래스의 역할을 엄격하게 분리하여 코드의 가독성과 유지보수성을 크게 높였습니다.
- **실시간 페이즈(Phase) 전환 연출:** 보스의 남은 체력 비율(50%, 20%)을 실시간으로 계산하여, 화면 상단의 보스바(BossBar) 타이틀과 몬스터 머리 위 이름표 색상이 즉각적으로 변환되도록 구현했습니다. 각 페이즈 진입 시 '광역 넉백'과 '번개 강타(Last Resort)' 같은 고유 스킬이 발동해 레이드의 긴장감을 극대화합니다.
- **Paper Component API 도입 및 점진적 최신화:** 기존 Spigot의 구버전 문자열 포맷팅(`§`) 방식이 최신 버전에서 Deprecated(사용 경고) 처리된 것을 확인했습니다. 그래서 이번에 새로 기획한 보스 시스템부터는 Paper 서버에서 권장하는 최신 Adventure Component API를 적극적으로 적용해 봤고, 기존에 짜뒀던 다른 코드들도 이 최신 규격에 맞춰 하나씩 리팩토링해 나가고 있습니다.
- **트러블 슈팅 (Threshold Skipping 버그 해결):** 직접 타격해가며 테스트를 해보니, 강력한 무기로 때려서 체력 변환 구간(50%)을 순식간에 건너뛰면 변신 스킬이 씹히는 로직 오류를 발견했습니다. 이를 해결하기 위해 조건문에 체력 수치뿐만 아니라 '현재 보스의 이름표(상태)'까지 한 번 더 체크하도록 코드를 수정했습니다. 덕분에 딜이 한 번에 너무 크게 들어와도 꼬이는 일 없이 페이즈가 안정적으로 넘어가게 버그를 잡았습니다.

> **[보스 소환 및 다이내믹 페이즈 전환 검증]**
> ![보스 레이드 시스템 검증](images/boss_raid_system.gif)

---

### 19. SkBee FastBoard 기반 실시간 GUI 순발력 미니게임 구현
