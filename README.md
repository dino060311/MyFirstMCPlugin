...
- **객체 생명주기 관리**:: 여러 기능이 함께 사용하는 핵심 데이터들을 안전하게 넘겨주는 구조를 짜서, 데이터가 꼬이거나 서버가 불안정해지는 현상을 방지했습니다.
- **기능별 독립 리스너 설계**:
  - 모든 이벤트를 한곳에 모으지 않고, menu, npc, items 등 각 기능(도메인)별 패키지에 전용 리스너를 분리하여 배치했습니다.
  - 이를 통해 특정 기능 수정 시 해당 모듈만 확인하면 되는 높은 유지보수 편의성을 확보했습니다.
