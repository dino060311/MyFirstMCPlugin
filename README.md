...
상황극 및 미니게임 서버를 목표로 직업 시스템, 보스 레이드, NPC 상호작용, 컷신 연출,
PvP 미니게임, 실시간 UI 시스템 등을 설계하고 구현했습니다.
...
- **직업별 장비 자동화:** 추첨 완료 시 각 직업의 상징 아이템(마피아: 네더라이트 검, 경찰: 나침반, 의사: 불사의 토템, 시민: 빵)이 자동 지급되며, 경찰은 **직업 스캐너**, 마피아는 **야간 암살 능력**을 사용할 수 있도록 역할 기반 시스템과 연동했습니다.
...
- **상황극 몰입도 향상:** CCTV 카메라와 모니터를 연동하여 플레이어가 특정 장소를 원격으로 감시할 수 있는 보안 관제 시스템을 구현했습니다.
- **카메라 위치 저장 기능:** 관리자가 원하는 위치와 시선을 CCTV 카메라 위치로 등록할 수 있도록 설계했습니다.
- **실시간 시점 전환:** 모니터 블록을 클릭하면 관전자(Spectator) 모드로 전환되어 CCTV 카메라 시점에서 주변 상황을 확인할 수 있습니다.
- **상태 백업 및 복구:** CCTV 접속 전 플레이어의 위치와 게임모드를 저장하고, 종료 시 원래 상태로 안전하게 복구하도록 구현했습니다.
- **이동 제한 로직:** CCTV 시청 중 플레이어가 카메라 위치를 벗어나지 못하도록 이동을 제어하여 실제 감시 카메라와 유사한 사용자 경험을 제공했습니다.
- **실시간 UI 연출:** ActionBar와 Title을 활용하여 녹화 중(REC) 상태를 시각적으로 표시하고 몰입감을 강화했습니다.
- **예외 처리 및 안정성 확보:** 서버 종료, 강제 접속 종료 등 예외 상황에서도 상태 데이터가 정상적으로 정리되도록 설계했습니다.

> **[실시간 CCTV 시스템 작동 검증]**
> ![CCTV 시스템](images/cctv_system.gif)


### 럭키 슬롯 머신 시스템

- **코인 기반 카지노 미니게임 구현:** 플레이어가 **황금 코인**을 사용하여 슬롯머신을 이용할 수 있는 카지노형 미니게임을 구현했습니다. 상황극 서버 내 재화 소비 요소를 추가하여 반복 플레이 동기를 강화했습니다.
- **확률 기반 보상 시스템 설계:** 슬롯 결과에 따라 **석탄, 철괴, 금괴, 에메랄드, 다이아몬드 잭팟** 등 서로 다른 보상을 획득하도록 설계했으며, 희귀한 결과일수록 더 높은 가치를 제공하도록 구현했습니다.
- **전용 아이템 및 패키지 지급 로직:** GUI 메뉴를 통해 **슬롯머신 기계 아이템과 황금 코인 패키지**를 지급받을 수 있도록 구성했으며, 기존 소지 여부를 검사해 최초 지급과 추가 코인 보급 로직을 구분 처리했습니다.
- **GUI 및 실시간 연출 강화:** 슬롯머신 작동 과정에서 GUI와 사운드 효과를 활용하여 실제 카지노와 유사한 긴장감과 보상 경험을 제공하도록 UX를 설계했습니다.
- **예외 처리 및 중복 방지:** 중복 아이템 지급 여부를 검사하여 데이터 꼬임을 방지하고, 플레이어 편의성을 고려한 보충 지급 로직을 구현했습니다.

> **[럭키 슬롯 머신 작동 검증]**
> ![럭키 슬롯 머신](images/lucky_slot_machine.gif)
